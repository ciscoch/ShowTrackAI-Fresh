-- ============================================================================
-- USER STORAGE TABLE FOR SUPABASE STORAGE ADAPTER (FIXED VERSION)
...
DO $$ 
BEGIN
    DROP POLICY IF EXISTS "Users can manage their own storage" ON public.user_storage;
...
-- ============================================================================
-- CREATE POLICIES (Fresh creation)
-- ============================================================================

-- Main policy: Users can manage their own storage data
...
DROP TRIGGER IF EXISTS handle_updated_at_user_storage ON public.user_storage;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'user_storage'
  ) THEN
    RAISE NOTICE '✅ user_storage table created successfully';
  ELSE
    RAISE EXCEPTION '❌ Failed to create user_storage table';
  END IF;
END $$;
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '🎉 User Storage Setup Complete!';
  RAISE NOTICE '================================';
  RAISE NOTICE 'Table: user_storage created with RLS';
  RAISE NOTICE 'Indexes: Performance indexes added';
  RAISE NOTICE 'Functions: Backup/restore functions ready';
  RAISE NOTICE 'Policies: User access controls enabled';
  RAISE NOTICE '';
  RAISE NOTICE 'Ready for SupabaseStorageAdapter! ✅';
END $$;

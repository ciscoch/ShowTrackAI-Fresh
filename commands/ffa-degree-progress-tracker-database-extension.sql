...
DROP POLICY IF EXISTS "student_own_degree_progress" ON public.ffa_degree_progress;
DROP POLICY IF EXISTS "student_own_sae_projects" ON public.ffa_sae_projects;
DROP POLICY IF EXISTS "student_own_sae_records" ON public.ffa_sae_records;
DROP POLICY IF EXISTS "student_own_competition_tracking" ON public.ffa_competition_tracking;
DROP POLICY IF EXISTS "student_own_analytics_events" ON public.ffa_analytics_events;
DROP POLICY IF EXISTS "student_own_user_interactions" ON public.ffa_user_interactions;
DROP POLICY IF EXISTS "educator_student_degree_progress" ON public.ffa_degree_progress;
DROP POLICY IF EXISTS "educator_student_sae_projects" ON public.ffa_sae_projects;
DROP POLICY IF EXISTS "educator_student_competition_tracking" ON public.ffa_competition_tracking;
DROP POLICY IF EXISTS "motivational_content_access" ON public.ffa_motivational_content;

-- Student Data Access Policies
CREATE POLICY "student_own_degree_progress" ON public.ffa_degree_progress
FOR ALL TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "student_own_sae_projects" ON public.ffa_sae_projects
FOR ALL TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "student_own_sae_records" ON public.ffa_sae_records
FOR ALL TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "student_own_competition_tracking" ON public.ffa_competition_tracking
FOR ALL TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "student_own_analytics_events" ON public.ffa_analytics_events
FOR ALL TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "student_own_user_interactions" ON public.ffa_user_interactions
FOR ALL TO authenticated
USING (auth.uid() = user_id);

-- Educator Access Policies (can view students in their organization)
CREATE POLICY "educator_student_degree_progress" ON public.ffa_degree_progress
...
CREATE POLICY "educator_student_sae_projects" ON public.ffa_sae_projects
FOR SELECT TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM public.profiles up
...
CREATE POLICY "educator_student_competition_tracking" ON public.ffa_competition_tracking
FOR SELECT TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM public.profiles up
...
CREATE POLICY "motivational_content_access" ON public.ffa_motivational_content
FOR SELECT TO authenticated
USING (active = true);
...
DROP TRIGGER IF EXISTS handle_updated_at_ffa_degree_progress ON public.ffa_degree_progress;
DROP TRIGGER IF EXISTS handle_updated_at_ffa_sae_projects ON public.ffa_sae_projects;
DROP TRIGGER IF EXISTS handle_updated_at_ffa_sae_records ON public.ffa_sae_records;
DROP TRIGGER IF EXISTS handle_updated_at_ffa_motivational_content ON public.ffa_motivational_content;
DROP TRIGGER IF EXISTS handle_updated_at_ffa_competition_tracking ON public.ffa_competition_tracking;
DROP TRIGGER IF EXISTS handle_updated_at_ffa_analytics_events ON public.ffa_analytics_events;
DROP TRIGGER IF EXISTS handle_updated_at_ffa_user_interactions ON public.ffa_user_interactions;

CREATE TRIGGER handle_updated_at_ffa_degree_progress 
BEFORE UPDATE ON public.ffa_degree_progress
FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

CREATE TRIGGER handle_updated_at_ffa_sae_projects 
BEFORE UPDATE ON public.ffa_sae_projects
FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

CREATE TRIGGER handle_updated_at_ffa_sae_records 
BEFORE UPDATE ON public.ffa_sae_records
FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

CREATE TRIGGER handle_updated_at_ffa_motivational_content 
BEFORE UPDATE ON public.ffa_motivational_content
FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

CREATE TRIGGER handle_updated_at_ffa_competition_tracking 
BEFORE UPDATE ON public.ffa_competition_tracking
FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

CREATE TRIGGER handle_updated_at_ffa_analytics_events 
BEFORE UPDATE ON public.ffa_analytics_events
FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

CREATE TRIGGER handle_updated_at_ffa_user_interactions 
BEFORE UPDATE ON public.ffa_user_interactions
FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();
